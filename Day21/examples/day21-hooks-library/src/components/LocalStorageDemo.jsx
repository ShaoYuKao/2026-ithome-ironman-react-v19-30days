...
function LocalStorageDemo() {
  const [note, setNote] = useLocalStorage('day21-sticky-note', DEFAULT_NOTE)
  // 同一個 Hook、同一個檔案，也可以在同一個元件裡再呼叫第二次、存不同的
  // key、存不同形狀的資料（這裡存的是數字），驗證「可重複使用」不只是
  // 口號，而是同一份程式碼真的能被安全地多次呼叫。
  const [visitCount, setVisitCount] = useLocalStorage('day21-visit-count', 0)
  ...
  return (
    <section className="card">
      <h2>2️⃣ useLocalStorage：會自動保存的便利貼</h2>
      <p className="card-desc">
        延續自 <a href="../Day13/README.md">Day13</a>，把 <code>useState</code> 換成{' '}
